...
Examples:
  ...
  %(prog)s "Ming Tai-Seale" --send-email --email-to recipient@email.com --email-from your@gmail.com --email-format html
    ...
    parser.add_argument(
        '--email', '-e',
        type=str,
        help='Your email (optional, recommended by NCBI)'
    )
    ...
    if args.output:
        html_filename = args.output if args.output.endswith('.html') else f"{args.output}.html"
    else:
    ...
    # Count DFM faculty
    faculty_count = len(set(
        author for pub in publications 
    ...
    if args.send_email:
        if not args.email_to or not args.email_from:
            print("❌ Error: --email-to and --email-from are required when using --send-email")
            return
