...
def main():
    ...
    if publications and "faculty_name" in publications[0]:
        queried_faculty = list(
            set(pub["faculty_name"] for pub in publications if pub.get("faculty_name"))
        )

    # Generate report
    generator = ReportGenerator(args.faculty_file)
    generator.generate_html_report(
        publications, args.output, title=args.title, queried_faculty=queried_faculty
    )
