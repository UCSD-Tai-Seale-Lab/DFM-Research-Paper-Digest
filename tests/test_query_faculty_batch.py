...
def test_run_query_DBMI_faculty_batch(username, logger, dbmi_faculty_webpage):
    output_file: str = (
        r"C:\Family Medicine\Publication Output\results\DBMI_batch_results.html"
    )
    Path(output_file).resolve().unlink(missing_ok=True)

    html: str = run_batch_report(
        contact_email=f"{username}@ucsd.edu",
        faculty_list_file=dbmi_faculty_webpage,
        log=logger,
        title="DBMI Faculty Publications Report (2026)",
        year=2026,
    )
    assert isinstance(html, str)
    ReportGenerator.write_html_file(html, output_file)
...
def test_run_query_DFM_faculty_batch(username, logger, dfm_faculty_webpage):
    output_file: str = (
        r"C:\Family Medicine\Publication Output\results\DFM_batch_results.html"
    )
    Path(output_file).resolve().unlink(missing_ok=True)

    html: str = run_batch_report(
        contact_email=f"{username}@ucsd.edu",
        faculty_list_file=dfm_faculty_webpage,
        log=logger,
        year=2026,
    )
    assert isinstance(html, str)
