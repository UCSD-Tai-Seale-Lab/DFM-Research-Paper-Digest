...
def query_faculty_batch(
    ...
    for i, faculty in enumerate(parsed_faculty, 1):
        ...
        pubmed_name = faculty["pubmed_long_format"]
        ...
        try:
            publications: list[dict] = query.query_author(pubmed_name, year=year)
            ...
            for pub in publications:
                ...
                if FacultyNameParser.check_middle_name_match(
                    pub["authors_list"], faculty
                ):
            ...
            faculty_results[original_name] = matching_publications
            all_results.extend(matching_publications)

            print(f"    Found: {len(matching_publications)} publication(s)")
